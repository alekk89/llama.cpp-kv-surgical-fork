...
struct sha256_state {
    ...
    void transform(const uint8_t * p) {
        ...
        for (int t = 0; t < 64; t++) {
            ...
            const uint32_t temp1 = hh + S1 + ch + K[t] + w[t];
...
std::string sha256_hex(const void * data, size_t len) {
    sha256_state st;
    if (len > 0) {
        st.process(reinterpret_cast<const uint8_t *>(data), len);
    }
    const auto digest = st.finalize();

    static const char * hexd = "0123456789abcdef";
    std::string res;
    res.reserve(64);
    for (uint8_t b : digest) {
        res.push_back(hexd[b >> 4]);
        res.push_back(hexd[b & 0xf]);
    }
    return res;
}
...
size_t server_tokens::get_common_prefix(const server_tokens & b) const {
    ...
    for (size_t i = 0; i < max_idx; ++i) {
        ...
        if (ai == LLAMA_TOKEN_NULL && bi == LLAMA_TOKEN_NULL) {
            const auto a_it =   map_idx_to_media.find(i);
            const auto b_it = b.map_idx_to_media.find(i);

            // Managed text surgery uses LLAMA_TOKEN_NULL as a physical hole.
            // On an mmproj server it is still a hole when no media chunk starts
            // at this index, and identical holes belong to the cached prefix.
            if (a_it == map_idx_to_media.end() || b_it == b.map_idx_to_media.end()) {
                if (a_it == map_idx_to_media.end() && b_it == b.map_idx_to_media.end()) {
                    continue;
                }
                return i;
            ...
            const auto & a_chunk = a_it->second;
            const auto & b_chunk = b_it->second;
